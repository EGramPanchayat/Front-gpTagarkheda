...
const NewsSection = () => {
  ...
  return (
    <section className="w-full bg-white border-y-2 border-y-orange-400 py-6 md:px-20 mb-13 md:mb-20  flex flex-col md:flex-row items-center overflow-hidden">
      ...
      <marquee className="text-green-800 font-medium text-base">
        {newsItems.length > 0 ? (
          newsItems.map((item, idx) => (
            <span key={item._id || idx} className="mx-6 text-2xl font-bold text-orange-400">
              • {item.text}
            </span>
          ))
        ) : (
